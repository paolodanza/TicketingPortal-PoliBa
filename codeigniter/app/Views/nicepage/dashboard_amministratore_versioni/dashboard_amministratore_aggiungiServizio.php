...
<html style="font-size: 16px;" lang="it"><head>
    ...
    <title>dashboard_amministratore_aggiungiServizio</title>
...
<link rel="stylesheet" href="<?php echo base_url();?>assets/dashboard_amministratore.css" media="screen">
    ...
    <meta property="og:title" content="dashboard_amministratore_aggiungiServizio">
...
<div id="formServizi"></div> <!-- PORTA LA PAGINA QUI --!>	            
			...
			<div class="u-expanded-width u-table u-table-responsive u-table-1" style="margin-top:40px;">
              ...
              <table class="u-table-entity">
                ...
                <thead class="u-align-center u-custom-font u-heading-font u-palette-4-base u-table-header u-table-header-1">
                  <tr style="height: 46px;">
                    <th class="u-border-2 u-border-palette-4-base u-table-cell">ID Servizio</th>
                    <th class="u-border-2 u-border-palette-4-base u-table-cell">Nome</th>
                    <th class="u-border-2 u-border-palette-4-base u-table-cell">Url</th>
                    <th class="u-border-2 u-border-palette-4-base u-table-cell">Responsabile</th>
                  </tr>
                </thead>
                <tbody class="u-table-body u-table-body-1">			
					<tr style="height: 50px;">
						<td class="u-border-2 u-border-palette-4-base u-table-cell">							
							...
							<?= csrf_field() ?>			
							<input type="input" value="<?= session()->getFlashdata('old_id');?>" style="width: 100%;" name="id_servizio"/><br/>								
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_nome');?>" placeholder="Max 100 caratteri" style="width: 100%;" name="nome"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_url');?>" placeholder="Max 1000 caratteri" style="width: 100%;" name="url"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<select name="cf_responsabile" id="cf_responsabile" style="width:100%;" class="u-border-3 u-border-palette-4-base u-input u-input-rectangle u-input-2">
								<?php foreach ($responsabili as $responsabile) {
									...
									if(is_null(session()->getFlashdata('old_cf')))
									{
										$selected = '';
									}else
									{
										$selected = (session()->getFlashdata('old_cf') == $id_responsabile_i) ? 'selected' : '';
									}									
				...
				<p style="width:150%;margin:0px;color: red;font-size:0.9rem;margin-top: 0px;margin-bottom: 0px;" class="u-align-left u-small-text u-text u-text-variant u-text-2">			  
					<?php if (session()->getFlashdata('errore_invio_aggiungi_servizio_mancante')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_servizio_mancante')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_servizio_mancante'); ?>					
					<?php endif; ?>					
					<?php if (session()->getFlashdata('errore_invio_aggiungi_servizio_id')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_servizio_id')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_servizio_id'); ?>							
						<?php session()->remove('old_id'); ?>
					<?php endif; ?>	
					<?php if (session()->getFlashdata('errore_invio_aggiungi_servizio_nome')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_servizio_nome')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_servizio_nome'); ?>
						<?php session()->remove('old_nome'); ?>
					<?php endif; ?>					

					<?php if (session()->getFlashdata('errore_invio_aggiungi_servizio_url')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_servizio_url')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_servizio_url'); ?>
						<?php session()->remove('old_url'); ?>								
					<?php endif; ?>
				</p>				
            </div>	
			<div style="display: flex; justify-content: center; gap: 50px; margin-top: 0px;margin-bottom: 5px;">
			  <button type="submit" name="submit" value="invia_aggiungi_servizio" style="margin-left: auto;margin-right: auto; border-style: solid; font-weight: 700; font-size: 1.25rem; --radius: 20px;"
			  ...
			  <button type="submit" name="submit" value="annulla"
				style="margin-left: auto;margin-right: auto; border-style: solid; font-weight: 700; font-size: 1.25rem; --radius: 20px;"
				class="u-active-custom-color-6 u-border-3 u-border-active-palette-4-base u-border-hover-palette-4-base u-border-palette-4-base u-btn u-btn-round u-button-style u-palette-4-base u-radius u-text-active-palette-4-base u-text-palette-5-base u-btn-1">
				Annulla
			  </button>
			</div>

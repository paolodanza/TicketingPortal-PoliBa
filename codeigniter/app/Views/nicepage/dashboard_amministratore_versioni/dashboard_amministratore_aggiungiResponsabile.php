...
<html style="font-size: 16px;" lang="it"><head>
    ...
    <title>dashboard_amministratore_aggiungiResponsabile</title>
...
<link rel="stylesheet" href="<?php echo base_url();?>assets/dashboard_amministratore.css" media="screen">
    ...
    <meta property="og:title" content="dashboard_amministratore_aggiungiResponsabile">
				  ...
				  <?php } ?>
					<tr style="height: 50px;">
						<td class="u-border-2 u-border-palette-4-base u-table-cell">							
							...
							<?= csrf_field() ?>			
							<input type="input" value="<?= session()->getFlashdata('old_cf');?>" placeholder="16 caratteri" style="width: 100%;" name="cf"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_nome');?>" placeholder="Max 100 caratteri" style="width: 100%;" name="nome"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_cognome');?>" placeholder="Max 100 caratteri" style="width: 100%;" name="cognome"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_email');?>" placeholder="Max 100 caratteri" style="width: 100%;" name="email"/><br/>		
						</td>
						<td class="u-border-2 u-border-palette-4-base u-table-cell">
							<input type="input" value="<?= session()->getFlashdata('old_password');?>" placeholder="Max 100 caratteri" style="width: 100%;" name="password"/><br/>		
						</td>		
					</tr>						
                </tbody>
              </table>
				<p style="width:150%;margin:0px;color: red;font-size:0.9rem;margin-top: 0px;margin-bottom: 0px;" class="u-align-left u-small-text u-text u-text-variant u-text-2">			  
					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_mancante')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_mancante')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_mancante'); ?>					
					<?php endif; ?>		
					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_cf')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_cf')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_cf'); ?>				
						<?php session()->remove('old_cf'); ?>						
					<?php endif; ?>	
					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_nome')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_nome')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_nome'); ?>	
						<?php session()->remove('old_nome'); ?>						
					<?php endif; ?>						
					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_cognome')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_cognome')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_cognome'); ?>					
						<?php session()->remove('old_cognome'); ?>
					<?php endif; ?>	
					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_email')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_email')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_email'); ?>
						<?php session()->remove('old_email'); ?>
					<?php endif; ?>					

					<?php if (session()->getFlashdata('errore_invio_aggiungi_responsabile_password')): ?>
						<?= session()->getFlashdata('errore_invio_aggiungi_responsabile_password')."<br>" ?>
						<?php session()->remove('errore_invio_aggiungi_responsabile_password'); ?>
						<?php session()->remove('old_password'); ?>								
					<?php endif; ?>
				</p>			  
            </div>
			<div style="display: flex; justify-content: center; gap: 10px; margin-top: 0px;">
			  <button type="submit" name="submit" value="invia_aggiungi_responsabile"
				class="u-active-custom-color-6 u-border-3 u-border-active-palette-4-base u-border-hover-palette-4-base u-border-palette-4-base u-btn u-btn-round u-button-style u-palette-4-base u-radius u-text-active-palette-4-base u-text-palette-5-base u-btn-1">
				Invia modifiche
			  </button>

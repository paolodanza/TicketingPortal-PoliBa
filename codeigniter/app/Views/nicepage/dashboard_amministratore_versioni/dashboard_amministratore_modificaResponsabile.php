...
<html style="font-size: 16px;" lang="it"><head>
    ...
    <title>dashboard_amministratore_modificaResponsabile</title>
...
<link rel="stylesheet" href="<?php echo base_url();?>assets/dashboard_amministratore.css" media="screen">
    ...
    <meta property="og:title" content="dashboard_amministratore_modificaResponsabile">
              ...
              <table class="u-table-entity">
                ...
                <tbody class="u-table-body u-table-body-1">
					<form action="<?php echo base_url('codeigniter/') ?>pagina_admin/modifica" method="post">
						<?= csrf_field() ?>	
						<?php for($i=0;$i<sizeof($responsabili);$i++){ ?>
						<tr style="height: 50px;">	
							<td class="u-border-2 u-border-palette-4-base u-table-cell">
								<input type="input" name="cf[]" value="<?= is_null(session()->getFlashdata('old_cf_'.$i)) ? esc($responsabili[$i]['cf']) : session()->getFlashdata('old_cf_'.$i) ?>" style="width: 100%;"/>
								<input type="hidden" name="cf_originale[]" value="<?= esc($responsabili[$i]['cf']) ?>" />
							</td >
							<td class="u-border-2 u-border-palette-4-base u-table-cell">
								<input type="input" name="nome[]" value="<?= is_null(session()->getFlashdata('old_nome_'.$i)) ? esc($responsabili[$i]['nome']) : session()->getFlashdata('old_nome_'.$i) ?>" style="width: 100%;"/>
								<input type="hidden" name="nome_originale[]" value="<?= esc($responsabili[$i]['nome']) ?>" />
							</td>
							<td class="u-border-2 u-border-palette-4-base u-table-cell">
								<input type="input" name="cognome[]" value="<?= is_null(session()->getFlashdata('old_cognome_'.$i)) ? esc($responsabili[$i]['cognome']) : session()->getFlashdata('old_cognome_'.$i) ?>" style="width: 100%;"/>
								<input type="hidden" name="cognome_originale[]" value="<?= esc($responsabili[$i]['cognome']) ?>" />
							</td>		
							<td class="u-border-2 u-border-palette-4-base u-table-cell">
								<input type="input" name="email[]" value="<?= is_null(session()->getFlashdata('old_email_'.$i)) ? esc($responsabili[$i]['email']) : session()->getFlashdata('old_email_'.$i) ?>" style="width: 100%;"/>
								<input type="hidden" name="email_originale[]" value="<?= esc($responsabili[$i]['email']) ?>" />
							</td>	
							<td class="u-border-2 u-border-palette-4-base u-table-cell">
								<input type="input" name="password[]" value="<?= is_null(session()->getFlashdata('old_password_'.$i)) ? esc($responsabili[$i]['passw']) : session()->getFlashdata('old_password_'.$i) ?>" style="width: 100%;"/>
								<input type="hidden" name="password_originale[]" value="<?= esc($responsabili[$i]['passw']) ?>" />
							</td>		
						</tr>
						<tr style="height: 50px;">
							...
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_mancante_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_mancante_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_mancante_'.$i); ?>					
							<?php endif; ?>					
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_cf_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_cf_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_cf_'.$i); ?>					
								<?php session()->remove('old_id_'.$i); ?>	
							<?php endif; ?>	
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_nome_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_nome_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_nome_'.$i); ?>
								<?php session()->remove('old_nome_'.$i); ?>
							<?php endif; ?>					
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_cognome_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_cognome_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_cognome_'.$i); ?>
								<?php session()->remove('old_descrizione_'.$i); ?>								
							<?php endif; ?>				
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_email_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_email_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_email_'.$i); ?>
								<?php session()->remove('old_email_'.$i); ?>								
							<?php endif; ?>				
							<?php if (session()->getFlashdata('errore_invio_modifica_responsabile_password_'.$i)): ?>
								<?= session()->getFlashdata('errore_invio_modifica_responsabile_password_'.$i)."<br>" ?>
								<?php session()->remove('errore_invio_modifica_responsabile_password_'.$i); ?>
								<?php session()->remove('old_password_'.$i); ?>								
							<?php endif; ?>				
        ...
        <div class="u-border-3 u-border-palette-4-base u-container-style u-group u-radius u-group-1" data-animation-name="" data-animation-duration="0" data-animation-delay="0" data-animation-direction="">
          <div class="u-container-layout u-container-layout-1">
			...
			<form action="" method="post" style="display: flex; justify-content: center;">
			  ...
			  <button type="submit" name="submit" value="aggiungi_tipologia"	style="flex: 0; margin-top: 35px; margin-left: 182px;"
			  ...
			  <button type="submit" name="submit" value="modifica_tipologia"	style="flex: 0; margin: 35px auto 0 auto;"
				class="u-active-custom-color-6 u-border-3 u-border-active-palette-4-base u-border-hover-palette-4-base u-border-palette-4-base u-btn u-btn-round u-button-style u-palette-4-base u-radius u-text-active-palette-4-base u-text-palette-5-base u-btn-1">
				Modifica
			  </button>
			  <button type="submit" name="submit" value="elimina_tipologia"	style="flex: 0; margin-top: 35px; margin-right: 194px; margin-left:auto"
